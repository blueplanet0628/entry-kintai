import { CardReaderDevice } from "./card-reader";
...
export class RCS380 implements CardReaderDevice {
	device: USBDevice;
	...
	public async getIDm() {
		...
		const idm = (await this.receive(37)).slice(17, 24);
		let idmStr = "";
	...
	private async receive(len: number) {
		log(`<<<<<<<<<<${len}`);
		const data = await this.device.transferIn(1, len);
		log(data);
